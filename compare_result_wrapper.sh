...
chmod +x *.sh 
chmod +x templatedir/*.sh

./check_objfn_v2b.sh ### gives dakota_output.xyz
no3=$(cat dakota_output.xyz | awk 'NR==3 {print NF}')
nolast=$(cat dakota_output.xyz | awk 'END {print NF}')
if [[ ("$no3" == 4 ) && ("$nolast" == 4 ) ]] ; then
    #echo dakota_output.xyz correctly generatedi
    zero=0
else
    #echo dakota_output.xyz NOT correctly generated
    chmod +x ./check_objfn_v2a.sh; ./check_objfn_v2a.sh ### gives dakota_output.xyz
    no3=$(cat dakota_output.xyz | awk 'NR==3 {print NF}')
    nolast=$(cat dakota_output.xyz | awk 'END {print NF}')
    if [[ ("$no3" == 4 ) && ("$nolast" == 4 ) ]] ; then
	    zero=0
    else
		./check_objfn_v2.sh ### gives dakota_output.xyz
    fi
fi
...
nolast=$(cat dakota_output.xyz | awk 'END {print NF}')
if [[ ("$no3" == 4 ) && ("$nolast" == 4 ) ]] ; then
    echo dakota_output.xyz correctly generated	
	./monitor_objf.sh    ### gives monitor_objf.dat, monitor_objf_sorted.dat
	
	python grep_LLS.py	 ### gives LLS_N_M_NOobj_fn_obj_fn.xyz
	
	./monitor_workdir.sh ### gives sigma_workdir.pdf or sigma_workdir.png, data.dat, minsigmavswdir.dat
